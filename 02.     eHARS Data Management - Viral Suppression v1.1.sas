...
*																		 ;
%let path = C:\Users\johnr\OneDrive\Desktop\Work\Road to Zero\Work\Simulation;
...
options dlcreatedir; 
libname out 			"&path.\Call &Sysdate.";
libname dataout 		"&path.\Call &Sysdate.\Data";
libname datasend 		"&path.\Call &Sysdate.\Datasend &Sysdate.";
libname output 			"&path.\Call &Sysdate.\Output &Sysdate.";
%let location_output =   &path;
filename 	log      	"&location_output\RTZ Data Management Log &Sysdate..log";

/*======================================================================*/
*				File location(s) of original datasets					 ;
/*======================================================================*/

/* This librefs need to be the location that the simulated data sets 	*/
/* are save to. There are multiple librefs as health deparments will 	*/
/* typically house these data sets in seperate folders					*/
libname person 	"C:\Users\johnr\OneDrive\Desktop\Work\Road to Zero\Work";
...
data Person_analysis_2;
	...
	if dthyr = . or DTHDATE > &censor_date. then do; 										
  	...
  	if 		df_facility_name = ' '   	then df_facility_name = 'Unknown';
  	if 	  care_facility_name = ' ' 		then care_facility_name = 'Unknown';

	if      df_facility_name = 'NOTF CRESCENTCARE/NOAIDSTASKFRC'							then dfname = 1;	
	else if df_facility_name = 'CHNO-ER MEDICAL CENTER OF LA AT NEW ORLEANS - CHARITY - ER'	then dfname = 2;
	else if df_facility_name = 'OP PVT1 REGION 1 OUTPATIENT PRIVATE PHYSICIAN'				then dfname = 3; 
	else if df_facility_name = 'OP PVT2 REGION 2 OUTPATIENT PRIVATE PHYSICIAN'				then dfname = 4;
	else if df_facility_name = 'OLOL-ER OUR LADY OF THE LAKE - ER'							then dfname = 5;
	else if df_facility_name = 'INS INSURANCE COMPANY'										then dfname = 6;
	else if df_facility_name = 'PP071 ORLEANS PARISH PRISON SYSTEM'							then dfname = 7;
	else if df_facility_name = 'OLOL OUR LADY OF THE LAKE REGIONAL MEDICAL CENTER'			then dfname = 8;
	else if df_facility_name = 'PHU017 CADDO PARISH HEALTH UNIT'							then dfname = 9;
	else if df_facility_name = 'DEL DELGADO STD CLINIC'										then dfname = 10;
	else if df_facility_name = 'MHEP2 METROPOLITAN HEALTH EDUCATION PROGRAM'				then dfname = 11;
	else if df_facility_name = 'PHILA7 PHILADELPHIA CENTER'									then dfname = 12;
	else if df_facility_name = 'CARES ACADIANA CARES'										then dfname = 13;
	else if df_facility_name = 'GOCARE8 GO CARE'											then dfname = 14;
	else if df_facility_name = 'BRGMC BATON ROUGE GENERAL MEDICAL CENTER'					then dfname = 15;
	else if df_facility_name = 'Unknown'													then dfname = 16;
	else																				 		 dfname = 17;
